...
export default function Royalties() {
  ...
  const processedData = useMemo(() => {
    ...
    const data = royaltyData.data;

    // Calculate platform revenue with percentages
    const regularPlatforms = data.platforms?.regular?.performance || [];
    const totalRegularRevenue = regularPlatforms.reduce((sum, p) => sum + (p.totalRevenue || 0), 0) || 1;

    const platformRevenue = regularPlatforms.map(platform => ({
      name: platform.platform,
      value: (platform.totalRevenue / totalRegularRevenue) * 100,
      color: PLATFORM_COLORS[platform.platform] || '#8B5CF6',
      amount: formatCurrency(platform.totalRevenue),
      share: `${((platform.totalRevenue / totalRegularRevenue) * 100).toFixed(1)}% share`,
  ...
  const platformRevenue = processedData?.platformRevenue || [];
  const performance = processedData?.performance || {};

  // Sample data for charts (used when API data is not available)
  const monthlyTrendsData = trends.monthlyRoyaltyTrends?.length > 0
    ? trends.monthlyRoyaltyTrends
    : [
  ...
  const platformRevenueData = platformRevenue.length > 0
    ? platformRevenue
    : [
    ...
    <div className="min-h-screen bg-background text-foreground p-6">
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="flex items-center gap-4">
          <Select
            ...
            onValueChange={(value) => setTimeframe(reverseTimeframeMap[value])}
          >
      ...
      {isLoading && (
        <div className="flex items-center justify-center h-64">
          <div className="text-muted-foreground">Loading royalty data...</div>
        </div>
      )}
      ...
      {error && (
        <div className="flex items-center justify-center h-64">
          <div className="text-red-500">Failed to load royalty data</div>
        </div>
      )}
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatCurrency(overview.totalEarnings || 0)}</div>
                <div className={`flex items-center text-sm ${overview.growthPercent >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                  <span>{overview.growthPercent >= 0 ? '+' : ''}{overview.growthPercent || 0}% from last period</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatCurrency(overview.thisMonthMoney || 0)}</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatCurrency(overview.bonusRoyalty || 0)}</div>
                <div className="flex items-center text-sm text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatCurrency(overview.regularRoyalty || 0)}</div>
                <div className="flex items-center text-sm text-muted-foreground">
                    ...
                    <div className="space-y-6">
                      <div className="flex items-center justify-between bg-muted-foreground/10 p-4 rounded-lg">
                        <div>
                          ...
                          <p className="text-xl font-bold">{formatCurrency(performance?.regular?.averageMonthly || 0)}</p>
                        </div>
                      ...
                      <div className="flex items-center justify-between bg-muted-foreground/10 p-4 rounded-lg">
                        <div>
                          ...
                          <p className="text-xl font-bold">
                            {formatCurrency(performance?.bonus?.bestMonth?.amount || 0)}
                            {performance?.bonus?.bestMonth?.month && ` (${performance.bonus.bestMonth.month})`}
                          </p>
                        </div>
                      ...
                      <div className="flex items-center justify-between bg-muted-foreground/10 p-4 rounded-lg">
                        <div>
                          ...
                          <p className={`text-xl font-bold ${overview.growthPercent >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                            {overview.growthPercent >= 0 ? '+' : ''}{overview.growthPercent || 0}%
                          </p>
                        </div>
                        <TrendingUp className="h-6 w-6 text-green-500" />
                      </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {platformRevenueData.map((platform, index) => (
                        <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {platform.units ? formatNumber(platform.units) + ' streams' : 'This period'}
                            </p>
                          </div>
                        </div>
                      ))}
