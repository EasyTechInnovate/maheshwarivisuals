...
import { mockMCNData } from "./MCNManagementData";
import { Upload, Edit, Trash2 } from "lucide-react";

export default function MCNManagement({ theme }) {
  const isDark = theme === "dark";
  const [search, setSearch] = useState("");
  const [data] = useState(mockMCNData);
  ...
  return (
    <div className={`p-4 md:p-6 space-y-6 transition-colors duration-300 ${isDark ? "bg-gray-900 text-gray-200" : "bg-gray-50 text-[#151F28]"}`}>
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-3">
        <div>
          <h1 className="text-xl font-semibold">MCN Management</h1>
            ...
            {showDropdown && (
              <div
                className={`absolute top-12 right-0 w-36 rounded-md shadow-md border z-20 ${
                  isDark ? "bg-[#151F28] text-gray-200 border-gray-700" : "bg-white text-gray-800 border-gray-200"
                }`}
              >
                <p
                  className={`px-3 py-2 cursor-pointer ${isDark ? "hover:bg-gray-700" : "hover:bg-gray-200"}`}
