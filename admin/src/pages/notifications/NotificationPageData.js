// NotificationPageData.js
export const notificationStats = {
  totalSent: { value: "24.7K", change: "+12.3%" },
  activeUsers: { value: "18.9K", change: "+8.7%" },
  openRate: { value: "72.4%", change: "+5.2%" },
  clickRate: { value: "23.8%", change: "+3.1%" },
};

export const mockNotification = {
  title: "Notification Title",
  message: "Your notification message will appear here...",
  audience: "All Users",
  type: "General",
};

export const mockUsers = [
  {
    id: 1,
    name: "Rajesh Kumar",
    email: "rajesh.kumar@maheshwarivisuals.com",
    phone: "7668009623",
    subscription: "Base",
    status: "Active",
    joinDate: "2023-01-15",
    lastActive: "2024-01-28",
  },
  {
    id: 2,
    name: "Priya Sharma",
    email: "priya.sharma@maheshwarivisuals.com",
    phone: "7668009623",
    subscription: "Base",
    status: "Active",
    joinDate: "2023-03-22",
    lastActive: "2024-01-28",
  },
  {
    id: 3,
    name: "Amit Patel",
    email: "amit.patel@maheshwarivisuals.com",
    phone: "7668009623",
    subscription: "Pro",
    status: "Active",
    joinDate: "2023-06-10",
    lastActive: "2024-01-27",
  },
  {
    id: 4,
    name: "Sneha Gupta",
    email: "sneha.gupta@maheshwarivisuals.com",
    phone: "7668009623",
    subscription: "Base",
    status: "Away",
    joinDate: "2023-08-05",
    lastActive: "2024-01-25",
  },
  {
    id: 5,
    name: "Vikram Singh",
    email: "vikram.singh@maheshwarivisuals.com",
    phone: "7668009623",
    subscription: "Base",
    status: "Inactive",
    joinDate: "2023-11-12",
    lastActive: "2024-01-20",
  },
];
