...
import { mockReleases } from "./ReleaseManagementData"; // Import the data file
...
export default function ReleaseManagement({ theme }) {
  ...
  const stats = [
    ...
    {
      label: "Under Review",
      value: releases.filter((r) => r.status === "Under Review").length,
    },
    {
      label: "Total Tracks",
      value: releases.reduce((sum, r) => sum + r.tracks, 0),
    },
  ];
            ...
            {filteredReleases.map((rel) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium ${
                      statusColors[rel.requestStatus]
                    }`}
                  >
                    {rel.requestStatus}
                  </span>
