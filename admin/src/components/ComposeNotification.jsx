...
export default function ComposeNotification({
  ...
  const handleSendNow = () => onCompose({ title, message, audience, type, when: "now" });
  const handleSchedule = () => onCompose({ title, message, audience, type, when: "scheduled" });
  ...
  {stats.slice(0, 2).map((stat, i) => (
    <div key={i} className={`${bgCard} p-4 rounded-2xl shadow text-left`}>
        ...
        <span
          className={`mt-2 inline-block text-xs font-medium px-2.5 py-1 rounded-full ${
            String(stat.change).trim().startsWith("+")
              ? i === 0
                ? "bg-green-900/30 text-green-400" // badge for first box
                : "bg-blue-900/30 text-blue-400"  // badge for second box
      ...
      <div className={`${bgCard} p-6 rounded-2xl shadow`}>
        ...
        <p className="text-gray-400 text-sm mb-4">Create and send notifications to users</p>
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className={`flex-1 ${theme === "dark" ? "bg-gray-700 hover:bg-gray-600" : "bg-gray-200 hover:bg-gray-300"} text-${theme === "dark" ? "white" : "black"} py-2.5 rounded-xl`}
          >
            ⏰ Schedule
          </button>
        </div>
