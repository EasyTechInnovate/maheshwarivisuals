...
export default function AnalyticsMonthManagement({ theme = "dark", onBack }) {
  ...
  const fetchMonths = async () => {
    try {
      setLoading(true);
      const res = await GlobalApi.getMonthsByType("analytics", false);
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold">Analytics Month Management</h1>
          <p
            className={`text-sm ${
              isDark ? "text-gray-400" : "text-gray-600"
            }`}
          >
            Organize monthly analytics months
          </p>
        </div>
